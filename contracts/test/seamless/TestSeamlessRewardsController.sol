...
/**
 * @title Seamless's RewardController Contract
 * @notice Hold and claim token rewards
 */
contract TestSeamlessRewardsController {
    struct RewardConfig {
        address rwdToken;
        uint256 amount;
    }
    ...
    function getUserRewards(
        ...
        for (uint256 i = 0; i < len_; i++) {
            ...
            for (uint256 j = 0; j < rewardData_.length; j++) {
                if (reward_ != rewardData_[i].rwdToken) {
                    continue;
                }
                totalReward_ += rewardData_[i].amount;
            }
        }
        return totalReward_;
    }
    ...
    function claimRewardsToSelf(
        ...
        for (uint256 i = 0; i < len_; i++) {
            ...
            for (uint256 j = 0; j < rewardData_.length; j++) {
                ...
                if (amount_ > rewardData_[i].amount) {
                    totalReward_ += rewardData_[i].amount;
                } else {
                    totalReward_ += amount_;
                }
            }
        }
        doTransferOut(reward_, msg.sender, totalReward_);
        emit RewardsClaimed(
            msg.sender,
            reward_,
            msg.sender,
